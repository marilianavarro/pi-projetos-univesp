...
/*COMPONENT*/
function bsignaturedrawcapture_start(id, value, penColor) {
    var canvas = document.getElementById(`canvas_${id}`);
    var signaturePad = new SignaturePad(canvas, { backgroundColor: 'rgb(255, 255, 255)'});
    
    signaturePad.penColor = penColor;

    var icon = $(`#${id}`).closest('.bsignaturedrawcapture_container').find('span');
    ...
    $(`#image_${id}`).css('background-image', `url(${value})`);

    $(`#${id}`).data('signature_pad', signaturePad);

    $(`#clear_${id}`).on('click', function() {
        $(`#${id}`).val('');
        $(`#image_${id}`).css('background-image', `url()`);
        signaturePad.clear();
        icon.show();
    });
    ...
    $(`#add_${id}`).on('click', function() {
        if (signaturePad.isEmpty()) {
            icon.show();
            $(`#image_${id}`).css('background-image', '');
            $(`#${id}`).val('');
        } else {
            icon.hide();
            $(`#image_${id}`).css('background-image', `url(${signaturePad.toDataURL()})`);
            $(`#${id}`).val(signaturePad.toDataURL());
        }
        $('#container_' + id).hide();
    });
    ...
    $(`#image_${id},#container_${id} + input + img + span`).on('click', function() {
        var width = $(window).width() * .90;
        var height = $(window).height() * .90;
        
        // Controle para saber se esta em modo retrato
        if (width < height) {
            __adianti_error('Atenção', 'Para assinar o documento é necessário estar em modo paisagem');
            return
        }
        
        $('#container_' + id + ' .bsignaturedrawcapture_actions').css({top: `calc( 10% + ${(height - 40)}px )` })
        $('#canvas_' + id ).css({width, height});
        $('#container_' + id).show();
        resizeCanvas();
    })
    const resizeCanvas = function() {
        ...
        signaturePad.clear();
        signaturePad.fromDataURL($(`#${id}`).val());
    }
